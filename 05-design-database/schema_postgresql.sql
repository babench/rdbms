...
COMMENT ON COLUMN otus.order.updated_time IS 'last updated timestamp; it is a timestamp with a time zone defines an exact moment when the data had updated';
...
COMMENT ON COLUMN otus.order_log.delivered_time IS 'actual delivery date and time; helps to define a date and time in a concrete time zone';
