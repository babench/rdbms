...
CREATE OR REPLACE FUNCTION next_store_order(VARCHAR(512), INT, VARCHAR(50)) RETURNS TEXT AS
$$
DECLARE
...
BEGIN
    ...
    END IF;
    RAISE NOTICE 'product_id = %', _product_id;
    ...
    END IF;
    RAISE NOTICE 'account_id = %', _client_id;
    ...
    -- make a new order
    INSERT INTO otus.order (owner_id, product_id, status, created_date, scheduled_date)
    VALUES (_client_id, _product_id, 'not_paid', now(), now() + interval '3 day') RETURNING id INTO _order_id;
    RAISE NOTICE 'order_id = %, status = not_paid', _order_id;

    INSERT INTO otus.order_details (order_id, product_id, comment, address, count, total_price, created_date)
    VALUES (_order_id, _product_id, 'comment', 'address', _order_product_count, _product_price, now());

    INSERT INTO otus.order_log (order_id, modified_by, status, created_date)
